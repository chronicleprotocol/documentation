...
**Following an offboarding, the affected oracle is no longer updated or technically maintained. Updates permanently cease and the value returned by the oracle is eventually set to 0**. This is a material technical state change. Any User must implement appropriate freshness checks, heartbeat monitoring, circuit breakers and fallback logic prior to implementation of an oracle and comply with the further offboarding procedure as set out in this document and as communicated by Chronicle to handle the post-offboarding state safely.
